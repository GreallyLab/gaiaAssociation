...
def gaiaAssociation(atacLocation, gwasLocation, chromosomeSize, outputLocation, uniqueCount=0, lociCutoff=0, lociSelection = 0, subsettingRegion=0, windowSize = 100000, overlapSaveValue=1):
    ...
    if lociSelection == 0:
        ...
        for item in set(gwasFormatted["DT"]):
            ...
            loopFrame = loopFrame[['CHR_ID', 'Start', 'End']]
            loopFrame = loopFrame.rename(columns={"CHR_ID": "Chromosome"})
            
            ## drop na values
            loopFrame = loopFrame.dropna()
            
            ## drop non int values
            loopFrame[pd.to_numeric(loopFrame['Start'], errors='coerce').notnull()]
            ##loopFrame[loopFrame.Start == loopFrame.Start // 1]
            
            loopFrame[pd.to_numeric(loopFrame['End'], errors='coerce').notnull()]
            ##loopFrame[loopFrame.End == loopFrame.End // 1]
